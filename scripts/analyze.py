...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Move [container:]/tmp/unet-test/unet-test.xml "
    ...
    cppath = "/tmp/unet-test"
    if args.save_xml or scount == 0:
        cppath += "/unet-test.xml"
    if contid:
    ...
    if scount and args.results and not os.path.exists(args.results):
        ...
        if not contid:
            ...
            tresfile, tresname = tempfile.mkstemp(
                suffix=".xml", prefix="unet-test-", text=True
            )
    ...
    if os.path.exists(os.path.join(args.results, "unet-test.xml")):
        args.results = os.path.join(args.results, "unet-test.xml")
    ...
    for f in ttfiles:
        m = re.match(r"tt-group-(\d+)/unet-test.xml", f)
